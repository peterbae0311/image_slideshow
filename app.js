...
// ============================================================
const GOOD_TEXTS = [
  '봄비가 창문을 두드리던 그날 오후, 나는 처음으로 혼자라는 것이 외로움이 아니라 온전함일 수 있다는 걸 어렴풋이 알았다.',
  '오래된 책의 페이지를 넘길 때마다 누군가의 손길이 닿는 것 같아서, 나는 도서관에서만큼은 결코 혼자인 적이 없었다.',
  '그녀가 떠난 자리에 남겨진 찻잔 하나, 아직 온기가 남아 있어 나는 한참을 그 자리에서 움직이지 못했다.',
  '가을 저녁의 냄새는 설명할 수가 없다. 낙엽과 흙과 멀리서 오는 연기가 뒤섞인 그 냄새는, 이상하게도 늘 안도감을 준다.',
  '그는 말이 없는 사람이었지만, 옆에 있으면 세상이 조금 더 조용하고 따뜻해지는 기분이 들었다.',
  '강물은 바위를 부수지 않는다. 다만 오랜 시간을 흘러 그것을 부드럽게 만들 뿐이다.',
  '첫눈이 내리던 밤, 우리는 아무 말도 하지 않고 그냥 나란히 앉아 창밖을 바라보았다. 그것만으로도 충분했다.',
  '삶이란 완성된 그림이 아니라, 매일 조금씩 덧칠해 가는 수채화 같은 것이라고 그녀는 말했다.',
  '저녁 햇살이 부엌 창을 비스듬히 물들이는 그 짧은 순간, 나는 이 작은 일상이 사실은 기적임을 느꼈다.',
  '기억은 사진보다 정직하다. 사진은 순간을 담지만, 기억은 그때의 냄새와 온도까지 함께 간직한다.',
  '그 사람이 웃는 법을 잃어버렸을 때, 나는 매일 조금씩 그 웃음을 다시 찾아줄 이유를 만들었다.',
  '어머니의 손은 항상 내 이마보다 조금 더 차가웠고, 그 차가움이 묘하게도 세상에서 가장 따뜻한 것이었다.',
  '파도는 물러나는 것처럼 보이지만, 사실 더 큰 파도를 만들기 위해 힘을 모으는 중이다.',
  '소년은 별을 세다 잠들었고, 꿈속에서도 별들은 그의 주변을 가만히 돌고 있었다.',
  '우리가 나눈 가장 긴 대화는 아무 말도 하지 않고 마주 앉아 같은 음악을 듣던 그 오후였다.',
  '낡은 골목에 접어들면 늘 시간이 천천히 흐르는 것 같았다. 그곳만큼은 세상이 나를 서두르게 하지 않았다.',
  '겨울 새벽의 공기는 날카롭지만 정직하다. 그 공기를 마시면 내가 살아 있다는 사실이 선명하게 느껴진다.',
  '그녀는 편지를 쓸 때면 항상 창가에 앉았다. 빛이 있어야 글씨가 예뻐진다고, 그녀는 늘 그렇게 믿었다.',
  '사람은 자신이 기억하고 싶은 것만 기억한다고 했지만, 나는 잊고 싶은 것들이 오히려 더 오래 남았다.',
  '산길을 오르다 잠시 멈춰 아래를 내려다보면, 그제야 내가 얼마나 멀리 왔는지 알 수 있다.',
  '그의 일기는 짧고 솔직했다. 오늘 햇빛이 좋았다. 고양이가 무릎에 올라왔다. 그것으로 충분한 하루였다.',
  '비가 그친 직후의 세계는 모든 것이 조금 더 또렷해 보인다. 빛도, 냄새도, 나의 마음도.',
  '할머니의 손등에는 주름이 많았는데, 나는 그 주름들 하나하나가 살아온 이야기임을 나중에야 깨달았다.',
  '음악이 끝나고 잠시 이어지는 침묵 속에서, 나는 때때로 세상에서 가장 아름다운 소리를 들었다.',
  '그 사람이 고맙다고 말할 때 눈을 보는 방식이 있었다. 말보다 더 많은 것을 담은 눈빛이었다.',
  '늦은 밤 혼자 끓이는 라면은 배고픔보다 더 깊은 무언가를 채워준다. 그것이 무엇인지는 설명하기 어렵다.',
  '봄이 되면 강변에 나가 한참 앉아 있곤 했다. 물이 흐르는 소리만으로도 마음의 먼지가 씻기는 것 같았다.',
  '좋은 사람을 알아보는 방법은 간단하다. 그 사람 곁에 있을 때 내가 더 좋은 사람이 되고 싶어지는지 보면 된다.',
  '나는 여행지에서 산 기념품보다 그곳의 공기, 골목, 낯선 언어의 소리를 더 오래 기억한다.',
  '등대는 폭풍 속에서도 자리를 지킨다. 그것이 존재의 이유이기 때문에, 흔들리지 않는 것이 아니라 흔들려도 켜져 있는 것이다.',
  '그해 여름, 우리는 선풍기 소리를 배경 삼아 많은 이야기를 나눴다. 그 이야기들은 지금도 어딘가 바람 속에 떠돌고 있을 것이다.',
  '새벽 네 시의 도시는 낮과는 다른 얼굴을 하고 있다. 조용하고 솔직하고, 조금은 슬프다.',
  '꽃이 지는 것을 슬퍼하지 않는다. 그것은 끝이 아니라, 다음 봄을 위한 가장 성실한 준비이므로.',
  '책 속의 어떤 문장은 오래 전에 쓰였는데도 꼭 오늘의 나를 위해 쓰인 것처럼 읽힐 때가 있다.',
  '그녀의 목소리는 낮고 조용했지만, 한 번 들으면 오랫동안 귓가에 남는 종류의 목소리였다.',
  '길을 잃었을 때 가장 좋은 방법은 서두르지 않는 것이다. 천천히 주변을 보면, 길은 언제나 거기 있다.',
  '사람이 사람에게 기댈 수 있다는 것, 그 단순한 사실이 때로는 세상에서 가장 큰 위안이 된다.',
  '가장 오래된 나무는 뿌리가 깊다. 폭풍이 지나가도 그 자리에 남아 있는 것들은 대개 깊이 뿌리박은 것들이다.',
  '그는 글을 쓸 때 항상 펜을 먼저 종이에 댄 채로 한참을 생각했다. 그 정직한 망설임이 좋았다.',
  '어린 시절의 나는 하늘이 이렇게 넓은 줄 몰랐다. 어른이 되어서야 올려다볼 여유를 잃은 탓이다.',
  '두 사람이 같은 방향을 바라볼 때, 그것이 사랑의 시작일 수 있다고 누군가 말했다.',
  '바람이 나뭇잎을 흔드는 소리는 지구가 숨을 쉬는 소리라고, 어린 나는 진지하게 믿었다.',
  '겨울이 깊을수록 봄은 더 간절하다. 그래서 봄꽃은 언제나 그토록 눈부신 것이다.',
  '오래된 장소에는 켜켜이 쌓인 시간의 냄새가 있다. 그 냄새 속에서 나는 늘 무언가를 되찾는 기분이 든다.',
  '말하지 않아도 알아주는 사람이 있다는 것, 그것이 가장 오래된 형태의 사랑일지 모른다.',
  '강은 굽어 흐르면서도 결국 바다에 닿는다. 돌아가는 길이 틀린 길이 아님을 강은 온몸으로 보여준다.',
  '첫사랑은 돌아오지 않아서 아름다운 게 아니라, 그 시절의 우리가 그토록 순수했기 때문에 아름다운 것이다.',
  '저녁 연기가 피어오르는 굴뚝을 보면 왜인지 모르게 마음이 놓인다. 거기 누군가 있다는 것이, 그것만으로도.',
  '글을 읽는다는 것은 타인의 마음속으로 잠시 이사 드는 일이다. 그리고 돌아올 때 우리는 조금 달라져 있다.',
  '여름밤의 풀벌레 소리는 지구의 오래된 자장가다. 그 소리를 들으면 잠이 오기 전에 먼저 마음이 잠든다.',
  '그 사람이 떠난 뒤에도 그 사람이 좋아하던 노래는 여전히 흘러나왔다. 음악은 사람보다 오래 남는다.',
  '사랑한다는 말을 아끼는 사람이 있다. 그것은 냉정함이 아니라, 그 말의 무게를 너무 잘 아는 까닭이다.',
  '어떤 날의 오후는 유난히 천천히 흐른다. 그런 날에는 아무것도 하지 않고 그냥 그 흐름에 실려 있어도 된다.',
  '눈이 내리는 밤은 세상이 한 겹 더 조용해진다. 그 고요 속에서 마음속 오래된 것들이 한 번씩 고개를 든다.',
  '나는 그 사람의 발소리만 들어도 알 수 있었다. 사랑이란 그런 것이다. 몸이 먼저 기억하는 것.',
  '삶의 아름다움은 완벽한 날들에 있지 않다. 허술하고 평범한 날들 사이에서 문득 빛나는 순간들에 있다.',
  '아이는 바다를 보며 무한히 경이로워하고, 어른은 그 같은 바다를 보며 저 너머를 생각한다. 그 차이가 나이다.',
  '사계절이 바뀌어도 같은 자리에 서 있는 나무처럼, 변하지 않는 것들이 있어 세상은 살 만하다.',
  '하루가 끝날 때, 오늘 나는 누구에게 친절했는가를 떠올려 본다. 그것만으로도 하루를 정리하기에 충분하다.',
  '그녀가 남긴 마지막 편지는 짧았다. 그러나 그 짧음 속에 오랜 시간 하지 못했던 모든 말들이 담겨 있었다.',
  '꿈은 이루어지기 전까지 빛난다고 했지만, 나는 이루어지지 않은 꿈들도 제 나름의 방식으로 빛을 낸다고 믿는다.',
];
